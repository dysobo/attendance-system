...
- 数据导出升级：月度导出改为 Excel 多工作表，支持按勾选顺序生成编号和统计表
- 数据统计升级：新增正餐、大夜班、小夜班、交通费、调休、加班等汇总列，并按工作日/休息日规则自动计算
...
- 仪表盘汇总卡片
- 年度加班/调休热力图
- 月度 Excel 导出，包含原始记录、考勤统计、数据统计 3 个工作表
- 备份导出与导入
...
- 月度导出从 CSV 升级为 Excel 多工作表
- 导出页支持勾选组员并调整导出顺序
- `考勤统计` 工作表新增姓名列并修正实出勤公式
- `数据统计` 工作表新增正餐、大夜班、小夜班、交通费、调休、加班等汇总列
- 夜班统计调整为可叠加口径，大夜班发生时可同时累计小夜班
- 首页右下角版本号升级为 V5.0
- 后端 API 版本升级为 5.0.0
